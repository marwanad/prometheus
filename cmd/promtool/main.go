...
func main() {
	...
	checkSyntaxCmd := checkCmd.Command("syntax", "Check if the config files have a valid syntax or not.")
	syntaxFiles := checkSyntaxCmd.Arg(
		"config-files",
		"The config files to check for valid syntax.",
	).Required().ExistingFiles()
...
func CheckSyntax(files ...string) int {
	...
	for _, f := range files {
		...
		if err != nil {
			...
			fmt.Printf("  SUCCESS: %s has a valid syntax", f)
		}
		fmt.Println()
	}
	if failed {
